  ...
  return (
    <nav className="bg-sky-500 text-white">
      <div className="container mx-auto px-2 sm:px-4">
        <div className="flex justify-between items-center h-14 sm:h-16">
          <div className="flex items-center space-x-4 sm:space-x-8">
            <Link to="/" className="text-lg sm:text-xl font-bold text-white hover:opacity-80 transition-opacity flex items-center space-x-2">
              <div className="w-5 h-5 sm:w-6 sm:h-6 bg-sky-700 rounded"></div>
            ...
            {isTVRoute && (
              <div className="hidden md:flex items-center space-x-4 lg:space-x-6">
                <Link 
                  ...
                  className={`text-xs lg:text-sm font-medium transition-colors hover:text-sky-200 ${
                    isActive('/tracker/dashboard') || isActive('/dashboard') ? 'text-white' : 'text-sky-100'
                  }`}
                >
                  Dashboard
                </Link>
                <div className="relative group">
                  <span className="text-xs lg:text-sm font-medium text-sky-100 hover:text-sky-200 cursor-pointer">
                    Shows
                  </span>
                ...
                {user && (
                  <Link 
                    to="/tracker/admin" 
                    className={`text-xs lg:text-sm font-medium transition-colors hover:text-sky-200 ${
                      isActive('/tracker/admin') || isActive('/admin') ? 'text-white' : 'text-sky-100'
                    }`}
                  >
                    Admin
                  </Link>
            ...
            {isFinanceRoute && user && (
              <div className="hidden md:flex items-center space-x-4 lg:space-x-6">
                <Link 
                  to="/finance" 
                  className={`text-xs lg:text-sm font-medium transition-colors hover:text-sky-200 ${
                    isActive('/finance') ? 'text-white' : 'text-sky-100'
                  }`}
                >
                  Dashboard
                </Link>
                <Link 
                  to="/finance/wallets" 
                  className={`text-xs lg:text-sm font-medium transition-colors hover:text-sky-200 ${
                    isActive('/finance/wallets') ? 'text-white' : 'text-sky-100'
                  }`}
                >
                  Wallets
                </Link>
                <Link 
                  ...
                  className={`text-xs lg:text-sm font-medium transition-colors hover:text-sky-200 ${
                    isActive('/finance/transactions') ? 'text-white' : 'text-sky-100'
                  }`}
                >
                  Transactions
                </Link>
                <Link 
                  ...
                  className={`text-xs lg:text-sm font-medium transition-colors hover:text-sky-200 ${
                    isActive('/finance/categories') ? 'text-white' : 'text-sky-100'
                  }`}
                >
                  Categories
                </Link>
                <Link 
                  to="/finance/reports" 
                  className={`text-xs lg:text-sm font-medium transition-colors hover:text-sky-200 ${
                    isActive('/finance/reports') ? 'text-white' : 'text-sky-100'
                  }`}
                >
                  Reports
                </Link>
          ...
          <div className="hidden md:flex items-center space-x-2 lg:space-x-4">
            {user ? (
              <>
                ...
                <span className="text-xs lg:text-sm truncate max-w-24 lg:max-w-32 text-sky-100">
                  {user.email}
                </span>
                <Button 
                  ...
                  variant="ghost"
                  size="sm"
                  className="text-xs lg:text-sm bg-sky-700 text-white hover:bg-sky-800 border-sky-700"
                >
                  Sign Out
                </Button>
              </>
              ...
              <div className="flex items-center space-x-2">
                <Link to="/sign-in">
                  <Button size="sm" className="text-xs lg:text-sm bg-sky-700 text-white hover:bg-sky-800">
                    Sign In
                  </Button>
                </Link>
                <Link to="/sign-up">
                  <Button size="sm" variant="outline" className="text-xs lg:text-sm border-sky-300 text-white hover:bg-sky-600">
                    Sign Up
                  </Button>
                </Link>
              </div>
          ...
          <div className="md:hidden">
            <Button
              ...
              className="text-white hover:bg-sky-600 p-1"
            >
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="md:hidden py-3 border-t border-sky-400">
            <div className="flex flex-col space-y-2">
              ...
              {user ? (
                <div className="pt-2 border-t border-sky-400">
                  <Link 
                    to="/profile" 
                    className="block text-sm font-medium text-sky-100 hover:text-white transition-colors py-1"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    Profile
                  </Link>
                  ...
                  <p className="text-sm mt-2 truncate text-sky-100">
                    {user.email}
                  </p>
                  <Button
                    variant="ghost"
                    ...
                    className="mt-2 w-full justify-start p-0 text-sm font-medium text-sky-100 hover:text-white transition-colors"
                  >
                    Sign Out
                  </Button>
                </div>
              ) : (
                <div className="pt-2 border-t border-sky-400 space-y-2">
                  <Link to="/sign-in" onClick={() => setIsMenuOpen(false)}>
                    <Button size="sm" className="w-full bg-sky-700 hover:bg-sky-800">
                      Sign In
                    </Button>
                  </Link>
                  <Link to="/sign-up" onClick={() => setIsMenuOpen(false)}>
                    <Button size="sm" variant="outline" className="w-full border-sky-300 text-white hover:bg-sky-600">
                      Sign Up
                    </Button>
                  </Link>
                </div>
